...
public function convertSalary(decimal salary, string sourceCurrency, string localCurrency) returns decimal|error {
    ...
    APIResponse response = check clientEndpoint->get(string`/rates/${sourceCurrency}`);

    if !response.rates.hasKey(localCurrency) {
        return error(string`cannot find key ${localCurrency}`);
    }
    return response.rates.get(localCurrency) * salary;
}
